...
int main(int argc, char*argv[])
{
	...
	glDepthFunc(GL_LESS); // (0, 1) GL_GREATER, GL_EQUAL, GL_GEQUAL, GL_ALWAYS
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        cameraHorizontalAngle -= dx * cameraAngularSpeed * dt; // dt is a constant. setting s
        cameraVerticalAngle   -= dy * cameraAngularSpeed * dt; 
        
        // Clamp vertical angle to [-85, 85] degrees
        cameraVerticalAngle = max(-85.0f, min(85.0f, cameraVerticalAngle)); //glm:clamp(val, min, max)
